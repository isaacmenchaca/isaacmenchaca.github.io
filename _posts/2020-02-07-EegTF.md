...
The code for the function can be located at: https://github.com/isaacmenchaca/isaacmenchaca-Computational-Cognitive-Neuroscience/blob/master/STFT/EegTF.py

Currently the stft4EegChannel function takes an EEG .mat file from EEGLAB as an input to retrieve appropriate information (sampling rate, time, channels, epoch trials), but the code can be adjusted to input this information manually -- something that can be done in the future. Currently, it is implemented to take the logarithm of the power (spectrum or density).
...
* **tf_log_power: np.array**: The power of the TF decomposition from desired time span or original input time span.
